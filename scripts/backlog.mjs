...
// stable TH-n id and a trailing `<!-- th: ... -->` metadata comment. ROADMAP.md and
...
const REPO_BLOB = 'https://github.com/Allan-Nava/trimhook/blob/main'
const DASH = ' — '
...
const LABELS = {
  hook: ['b7552f', 'The hook itself: trimming, spill, harness shapes'],
...
export function parse(text) {
  ...
  const flush = () => {
    ...
    const m = raw.match(/^- \[( |x)\] \*\*(TH-(\d+))\s+—\s+(.+?)\*\*:?\s*([\s\S]*)$/)
    if (!m) err(cur.line, 'item does not match `- [ ] **TH-n — Title**: body <!-- th: ... -->`')
    else {
      const hg = meta(raw, 'th')
...
export function lint(model) {
  ...
  for (const it of model.items) {
    ...
    if (!it.meta) {
      err(it.line, `${it.id} has no <!-- th: ... --> metadata`)
      continue
    }
...
export function roadmap(model) {
  ...
  const out = []
  out.push('# Roadmap — trimhook', '', '<!-- GENERATED by scripts/backlog.mjs roadmap — do not edit by hand. -->', '')
...
function existingIssues() {
  ...
  const map = new Map()
  for (const [id, num, state, title, milestone] of rows) if (/^TH-\d+$/.test(id)) map.set(id, { num, state, title, milestone })
  return map
}
...
\`BACKLOG.md\` is the single source of truth: it carries the stable \`TH-n\` id that commits and the CHANGELOG reference, and [ROADMAP.md](${REPO_BLOB}/ROADMAP.md) is generated from it. This issue is a view of that item, kept in step by \`scripts/backlog.mjs issues --apply\`, so closing it means ticking the item in the backlog and regenerating the roadmap in the same commit.
`
}
